@section('script_after')
    <script>
        $(function () {
            ...
            if(JSON.parse("{{ json_encode($expense_allowance->status_id) }}")===3){
...
@section('main')
    ...
    <div class="d-flex flex-wrap justify-content-between"><h4 class="">Algemene onkosten > Betaling</h4>
...
{{--geselecteerd--}}
        ...
        <Form id="expenseForm" action="/payment_review/{{$expense_allowance->id}}/update"  method="post" class="mb-5" enctype="multipart/form-data">
            @method('put')
            @csrf
